...
router.get('/revenue-monthly', async (req, res) => {
  try {
    const rows = await accountManager.withYouTube(async (auth) => {
      const ya  = google.youtubeAnalytics({ version: 'v2', auth })
      const now = new Date()
      const endDate   = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth() - 1, 1)).toISOString().split('T')[0]
      const startDate = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth() - 12, 1)).toISOString().split('T')[0]
